...
Была дописана еще одна функция tone(pin, function), которая принимает номер пина
пищалки и функцию, которая управляет звуком, в нашем случае она читает из памяти
по указателю, запомнив его у себя локально, частоту и задержку, перемещает указатель
на соответствующее количество байт и так в циклепроигрывает файл пока наша программа
не позовет функцию noTone().
